# Wave equation in 3d: Single-threaded version
# author: Lucas Romero Fernández 2025
# Start with "julia Solver_for_3D_wave_equation.jl"
...
function Boundcond!(kappa,nx,ny,nz,u,u₁)
    ...
    u[1,1:ny-1,1:nz-1] = u₁[2,1:ny-1,1:nz-1] + ((kappa - 1)/(kappa + 1))*(u[2,1:ny-1,1:nz-1] - u₁[1,1:ny-1,1:nz-1])
    u[nx,1:ny-1,1:nz-1] = u₁[nx-1,1:ny-1,1:nz-1] + ((kappa - 1)/(kappa + 1))*(u[nx-1,1:ny-1,1:nz-1] - u₁[nx,1:ny-1,1:nz-1])
    u[1:nx-1,1,1:nz-1] = u₁[1:nx-1,2,1:nz-1] + ((kappa - 1)/(kappa + 1))*(u[1:nx-1,2,1:nz-1] - u₁[1:nx-1,1,1:nz-1])
    u[1:nx-1,ny,1:nz-1] = u₁[1:nx-1,ny-1,1:nz-1] + ((kappa - 1)/(kappa + 1))*(u[1:nx-1,ny-1,1:nz-1] - u₁[1:nx-1,ny,1:nz-1])
    u[1:nx-1,1:ny-1,1] = u₁[1:nx-1,1:ny-1,2] + ((kappa - 1)/(kappa + 1))*(u[1:nx-1,1:ny-1,2] - u₁[1:nx-1,1:ny-1,1])
    u[1:nx-1,1:ny-1,nz] = u₁[1:nx-1,1:ny-1,nz-1] + ((kappa - 1)/(kappa + 1))*(u[1:nx-1,1:ny-1,nz-1] - u₁[1:nx-1,1:ny-1,nz])
end
